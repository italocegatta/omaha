...
Defines the rules that distinguish a trustworthy test suite from a
permissive one. A test that passes under a broken implementation is
worse than a missing test — it provides false confidence. This
capability governs the patterns that produce or prevent that
failure mode.
