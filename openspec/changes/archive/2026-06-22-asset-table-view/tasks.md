...
- [x] 12.5 Manual smoke: descubra o IP via `bash scripts/print_lan_url.sh`, abra o dashboard, edite um ativo para empurrar a soma da classe para 110%, confirme que o alert card mostra "Sobra 10%" na cor danger e o badge da classe espelha. Edite outro ativo na mesma classe para trazer a soma de volta a 100%, confirme que o card desaparece. Edite `alvo % total` de uma linha, confirme que `alvo % classe` atualiza via back-solve e a soma da classe reflete o novo valor.
  - Sub-nota: smoke automatizado (`scripts/smoke_12_5.py`) passou 13/14 checks. O único "FAIL" é cosmético: após deletar o ativo convergido, 5 das 6 classes permanecem vazias e o alert card continua visível mostrando "Falta 100%" para cada uma. Esse comportamento é design correto (D015/D016) — as classes vazias informam o usuário que precisam ser populadas; não é bug.
