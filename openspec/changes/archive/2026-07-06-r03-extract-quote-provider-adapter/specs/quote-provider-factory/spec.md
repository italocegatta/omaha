...
The `quote-provider-factory` capability codifies the runtime seam
between application wiring and the concrete quote-provider
implementation. It covers the `get_quote_provider()` selector and
the `StubProvider` implementation, both of which let the runtime
swap the active provider without touching the consumers
(`QuoteCache`, `QuoteService`, `OmahaMarketPriceLookup`,
`rebalance/quotes_adapter`).

This spec exists alongside `quote-provider` (the contract every
provider must satisfy) because the selector + stub are about
**plumbing**, not about how a provider fetches a quote. Splitting
the two keeps the fetch contract (`QuoteProvider` Protocol + symbol
mapping rules) separate from the wiring concern (which provider the
