...
- **AND** the asserted status code matches the route handler at
  `src/omaha/routes/assets.py:343`
...
When a route handler returns a different HTTP status code than a
test expects, the fix direction (edit the test vs. edit the code)
MUST be chosen by one of: a documented spec in `openspec/specs/`
or `.planning/`; a call-site that depends on the current
