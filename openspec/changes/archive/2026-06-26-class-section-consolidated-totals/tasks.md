...
- [x] 6.1 `uv run task db-reset` — confirm DB has Italo + 6 classes + 48 assets + 47 positions.
- [x] 6.2 `uv run task serve` — manual browser pass: login as Italo → land on `/` showing Italo's dashboard.
- [x] 6.3 Manual browser pass: for each class section, the consolidated `Valor`, `Alvo`, `Atual` pills (and `Sobra/Falta` when off) align visually with the `Valor`, `Alvo % total`, `Atual % total` columns (and `Alvo % classe` column) of the asset table below.
- [x] 6.4 Manual browser pass: collapse a class section — header (with all 5 cells) stays visible, asset table hides.
- [x] 6.5 Manual browser pass: hover the × button — still red, still darkens on hover.
- [x] 6.6 Manual browser pass: click `Alvo NN%` pill — inline editor opens (existing edit flow unchanged).
- [x] 6.7 DevTools pass: change `--col-ativo` value in `:root` → both header and table re-align on next layout (sanity check the single-source-of-truth contract).
- [x] 6.8 Mobile (resize browser <480px): header stacks gracefully; pills remain readable; × remains tappable.

## 7. Lint + check + delivery

- [x] 7.1 `uv run task check` (lint + test-unit) — green.
- [x] 7.2 `uv run task test-integration` — green.
- [x] 7.3 `uv run task test-e2e` — green.
