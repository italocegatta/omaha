...
Provides Omaha production favicon asset and shared head discovery contract.
...
`<head>` of `src/omaha/templates/base.html` with `rel="icon"`,
`type="image/svg+xml"`, and `href="/static/favicon.svg"`. Pages that extend
`base.html` SHALL inherit the same link; page-specific templates SHALL NOT
introduce alternate favicon links.
