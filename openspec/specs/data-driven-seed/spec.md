...
delete in order every `Position` whose `asset` belongs to
...
The seed script MUST, when invoked with `--mode upsert --profile {name}`,
NOT delete any rows. It MUST create classes that don't
...
The seed script MUST, when invoked with `--mode diff --profile {name}`,
read the CSV triplet (classes + assets + positions), run
