...
Define bounded, descriptive operational analysis of MyProfit synchronization telemetry.
...
`db-correlation-unavailable`, not reconstructed or joined by time, filename,
or profile label.
...
- **WHEN** stdout was not retained, rotation removed part of the requested
  window, fields are invalid, fewer than four weeks are available, a job has
  no terminal event, or no stated recurrence threshold is met
