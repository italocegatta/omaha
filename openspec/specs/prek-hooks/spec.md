...
The pre-push stage SHALL run the full pytest suite (`pytest` with no marker filter) as a blocking gate. The hook entry MUST be `./.venv/bin/python -m pytest` with `pass_filenames = false`.

#### Scenario: Full suite passes and push lands
- **WHEN** all 335 tests pass (or 335 pass + 2 skipped)
- **THEN** the pre-push hook reports success
- **AND** the `git push` proceeds

#### Scenario: Full suite fails and push is blocked
- **WHEN** one or more tests fail
- **THEN** the pre-push hook reports failure with the failing test names
- **AND** the `git push` is blocked
