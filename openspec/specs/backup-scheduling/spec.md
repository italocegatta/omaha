## Purpose
Periodic, unattended backup of the SQLite portfolio database in
production. The scheduler is a long-running sidecar service that fires
`sripts/backup.py` on a fixed interval (default 24 h), reuses the same
image and snapshot shape as the manual `backup` service, and tolerates
individual run failures without exiting. Operator retains control over
the interval, can stop the scheduler without touching `web`/`nginx`,
and relies on Docker's `restart: unless-stopped` to survive host
reboots.
## Requirements
