...
- **Catalog (D02 §Gate 1 + F12 D-F12.2 + F60 + R31)** — 12 nomes, scoped:
  ...
  - Import status — `check_circle` (matched) / `help` (unmatched).
   ...
   - Position sync — `sync` (em `_patrimonio_actions::dashboard-sync-btn`).
...
- Self-host vs Google Fonts é decisão de implementação (D-F12
 ...
 - **Extension path**: novos icons fora dos 12 nomes catalogados
  ...
  DESIGN.md + assertion no `tests/test_iconography_tokens.py`.
   Motivação: 12 cobre a surface atual; drift silencioso sem
