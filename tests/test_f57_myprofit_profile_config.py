"""Offline F58 coverage for profile-scoped MyProfit configuration."""
...
def test_settings_model_fields_and_environment_are_profile_specific(
    ...
    assert {field for field in Settings.model_fields if field.startswith("MYPROFIT_")} == (
        MYPROFIT_FIELDS
    )
...
def test_legacy_destination_environment_values_are_ignored(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MYPROFIT_ITALO_DESTINATION", "legacy-italo-destination-marker")
    monkeypatch.setenv("MYPROFIT_ANA_DESTINATION", "legacy-ana-destination-marker")
    config = _settings()

    resolved = resolve_myprofit_profile_config(_profile("Italo"), config)

    assert resolved.email == "italo.synthetic@fixture.test"
    assert not hasattr(config, "MYPROFIT_ITALO_DESTINATION")
    assert not hasattr(resolved, "destination")
    assert "legacy-italo-destination-marker" not in repr(config)
    assert "legacy-italo-destination-marker" not in repr(resolved)
...
def test_secret_sanitization(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.INFO):
        logging.getLogger("f58-test").info("resolved=%r settings=%r", resolved, config)
...
def test_env_example_placeholders_are_false() -> None:
    ...
    assert not any(field.endswith("_DESTINATION") for field in entries)
    assert len({entries[field] for field in MYPROFIT_FIELDS}) == 4
    assert entries["MYPROFIT_ITALO_EMAIL"].endswith("@example.invalid")
    assert entries["MYPROFIT_ANA_EMAIL"].endswith("@example.invalid")
    assert all(
        "example.invalid" in entries[field] or "replace-with-" in entries[field]
        for field in MYPROFIT_FIELDS
    )
...
def test_docs_do_not_contain_secrets() -> None:
    ...
    assert "MYPROFIT_ITALO_DESTINATION" not in readme
    assert "MYPROFIT_ANA_DESTINATION" not in readme
    for marker in (
