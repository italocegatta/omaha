...
def _login_and_select(client: TestClient, profile_id: int = 1) -> None:
    ...
    auto-binds the logged-in user's own first profile. Logging in
    as ``Italo`` already binds profile 1; logging in as ``Ana``
    already binds profile 2. The explicit ``/profiles/{id}/select``
    step only runs when the requested profile is owned by a
    different user.
    """
