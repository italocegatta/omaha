...
def _login_and_select(client: TestClient, profile_id: int = 1) -> None:
    ...
    auto-binds the logged-in user's own first profile. The seed
    creates profile 1 = Italo (display_order=0) and profile 2 =
    Ana (display_order=1); logging in as the matching user
    already binds the right profile. The explicit
    ``/profiles/{id}/select`` step only runs for cross-profile.
    """
